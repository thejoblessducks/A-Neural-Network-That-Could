'''This implementation is not ours, chaged lines: 137,155-170,186,191 to suit our purposes'''
'''We are only using it to hel show our Neural Nodes and connections'''
...
class TextOverlappingHandler():
    ...
    def getspace(self, test_coordinates):
        ...
        if self.cells[x_left_pos, y_botttom_pos] and self.cells[x_left_pos, y_top_pos] and self.cells[x_right_pos, y_top_pos] and self.cells[x_right_pos, y_botttom_pos]:
...
class NeuralNetwork():
    ...
    def draw(self,lr=None,weights_list=None):
        ...
        title = ''
        if lr is None:
            title = 'Neural Network architecture (Initial Network)'
            figureName='./Images/ANN_INITIAL_'+strftime("%Y%m%d_%H%M%S", localtime())+'.png'
        else:
            if lr == 'final':
                title = 'Neural Network architecture (Final Network)'
                figureName='./Images/ANN_FINAL_'+strftime("%Y%m%d_%H%M%S", localtime())+'.png'
            else:
                title = 'Neural Network architecture (Network learning rate '+lr+')'
                figureName='./Images/ANN_lr_'+lr+'_'+strftime("%Y%m%d_%H%M%S", localtime())+'.png'
...
class DrawNN():
    ...
    def __init__( self, neural_network, weights_list=None ):
        ...
        if weights_list is None:
            ...
            self.weights_list = weights_list
    def draw(self,lr=None):
        widest_layer = max( self.neural_network )
        network = NeuralNetwork( widest_layer )
        for l in self.neural_network:
            network.add_layer(l)
        network.draw(lr,self.weights_list)